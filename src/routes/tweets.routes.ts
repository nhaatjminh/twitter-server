...
import { userLoggedInValidator } from '~/controllers/users.controllers'
...
tweetsRouter.get(
  '/:tweet_id',
  tweetIdValidator,
  userLoggedInValidator(accessTokenValidator),
  userLoggedInValidator(verifiedUserValidator),
  audienceValidator,
  requestHandlerWrapper(getTweetController)
)
